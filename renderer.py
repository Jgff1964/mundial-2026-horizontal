...
def team_text(name: str) -> str:
    if not name: return "Rival a definir"
    if name.lower().startswith("ganador") or name.lower().startswith("perdedor") or "definir" in name.lower():
        return escape(name.upper())
    return f"{flag_for(name)} {escape(name.upper())}"
...
def render_svg(bracket, subtitle="Pantalla horizontal · zoom móvil"):
...
<text x="900" y="230" class="trophy">🏆</text>''']
    ...
    for no,y in zip([74,77,73,75,83,84,81,82],left_y):
        m=bracket["r32"][no]; color="#0b78dd" if no in [74,77,73,75,81,82] else "#00a0a8"; parts.append(box(x_l32,y,bw,bh,f"M{no} | {m['venue']}",m["home"],m["away"],m["sub"],color))
    for no,y in [(89,200),(90,380),(93,595),(94,780)]:
        m=bracket["r16"][no]; parts.append(box(x_l16,y,160,bh,f"M{no} | {m['venue']}",m["home"],m["away"],"", "#0b78dd" if no in [89,90] else "#00a0a8"))
    for no,y in [(97,330),(98,680)]:
        m=bracket["qf"][no]; parts.append(box(x_lqf,y,135,bh,f"M{no} | {m['venue']}",m["home"],m["away"],"", "#0b78dd" if no==97 else "#00a0a8"))
    ...
    for no,y in [(99,330),(100,680)]:
        m=bracket["qf"][no]; parts.append(box(x_rqf,y,135,bh,f"M{no} | {m['venue']}",m["home"],m["away"],"","#d11169"))
    for no,y in [(91,200),(92,380),(95,595),(96,780)]:
        m=bracket["r16"][no]; parts.append(box(x_r16,y,160,bh,f"M{no} | {m['venue']}",m["home"],m["away"],"","#e0004d"))
    for no,y in zip([76,78,79,80,86,88,85,87],right_y):
        m=bracket["r32"][no]; parts.append(box(x_r32,y,bw,bh,f"M{no} | {m['venue']}",m["home"],m["away"],m["sub"],"#e0004d"))
    ...
    parts.append('''<text x="900" y="980" class="foot">Pantalla horizontal · zoom móvil · datos FIFA</text><style>
