...
def resolve_seed(seed, zones, include_thirds=False):
    ...
    # Los terceros quedan en blanco hasta que el cuadro oficial/Promiedos los defina.
...
def make_bracket_from_zones(zones, include_thirds=False):
    out = {"r32":{}, "r16":{}, "qf":{}, "sf":{}, "third":{}, "final":{}, "source":"Promiedos API tablas"}
...
def overlay_promiedos_bracket(base, prom):
    ...
    if prom:
        for round_key in ["r32","r16","qf","sf","third","final"]:
            for no, data in prom.get(round_key, {}).items():
                ...
                h = canon_country_name(data.get("home"))
                a = canon_country_name(data.get("away"))
    ...
    base["source"] = "Promiedos cuadro armado" if changed else "Promiedos API tablas + estructura Promiedos"
