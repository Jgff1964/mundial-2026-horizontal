...
.primary{background:linear-gradient(90deg,#159cff,#ff2b6d)} .viewport{flex:1;overflow:auto;-webkit-overflow-scrolling:touch;touch-action:pan-x pan-y pinch-zoom;background:#02040d}
#canvas{transform-origin:top left;display:inline-block;min-width:1800px;min-height:1010px}.empty{padding:40px;color:#cad4ee;font-size:24px;line-height:1.3;min-width:900px}
#status{position:absolute;top:68px;left:8px;right:8px;z-index:5;background:rgba(7,13,31,.9);border:1px solid rgba(255,255,255,.16);border-radius:10px;padding:8px;font-size:12px;color:#dce6ff;display:none}
#status.show{display:block}.error{background:rgba(255,43,109,.22)!important;border-color:#ff2b6d!important}
.rotate{display:none}@media (orientation:portrait){.rotate{display:flex;position:fixed;inset:0;z-index:20;background:#050816;align-items:center;justify-content:center;text-align:center;padding:28px;font-size:24px;font-weight:800;line-height:1.25}}
</style></head><body>
...
<div id="status"></div><main class="viewport" id="vp"><div id="canvas">{{ svg|safe }}</div></main>
<script>
let zoom=1;const canvas=document.getElementById('canvas');const vp=document.getElementById('vp');
function showStatus(msg,err=false){const s=document.getElementById('status');s.textContent=msg;s.className='show'+(err?' error':'');setTimeout(()=>{s.className=''},5000);}
...
async function updateData(){showStatus('Actualizando desde Promiedos...');try{const r=await fetch('/api/update',{method:'POST'});const j=await r.json();if(!j.ok)throw new Error(j.error||'Error');await loadBracket();showStatus('Actualizado: '+j.last_update+' · '+j.mode);}catch(e){showStatus('Error: '+e.message,true);}}
...
def current_svg():
    ...
    subtitle = "Promiedos + zonas validadas" + (" · con terceros" if STATE["include_thirds"] else " · sin terceros")
